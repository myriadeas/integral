...
define(['angular', 'lodash','jquery','cataloguing2/cataloguing2', 'marc4js'], 
    ...
    module
      .controller('Tag007FormCtrl', function ($scope, $http, Schema, Form, Tag, MarcService2, Marc21TagForm, $modalInstance) {
        ...
        $scope.changeCategoryOfMaterial = function() {
            ...
            MarcService2.getSchemaAndForm($scope.tag.name, type).then(function(response) {
    ...
    module
      .controller('Tag006FormCtrl', function ($scope, $http, Schema, Form, Tag, MarcService2, Marc21TagForm, $modalInstance) {
        $scope.Marc21TagForm = Marc21TagForm;
    ...
    module.controller('ResourceDescriptorCreateCtrl', function ($scope, Templates, MarcService2, $location, Schema) {
        $scope.template = Templates[0];
        $scope.templates = Templates;
        $scope.select = function (template) {
            MarcService2.getTemplate(template).then(function(response) {
                var newRecord = marc4js.newRecord(response.data, Schema.data);
                MarcService2.save(newRecord).then(function(response) {
                    $location.path('cataloguing2/resourcedescriptor/' + parseInt(response.getId()) + '/edit');
    ...
    module.controller('ResourceDescriptorEditCtrl', function ($scope, $http, $modal, MarcService2, record, Marc21Form, flash, Localization, $location) {
        $scope.record = record;
        renderGrid();
            ...
            if(!angular.isUndefined(tag)){
                if(tag.name == "leader" || tag.name == "006" || tag.name == "007" || tag.name == "008") {
                    ...
                    var modalInstance = $modal.open({
                      templateUrl: 'views/cataloguing2/resourceDescriptor/tagForm.html',
                      controller: controller,
                      size: 'lg',
                      resolve: {
                            Schema: function(MarcService2) {
                                return  MarcService2.getSchema(tagName, tagType);
                            },
                            Form: function(MarcService2) {
                                return  MarcService2.getForm(tagName, tagType);
                            },
                            Tag: function() {
                                return tag.clone();
                            },
                            Marc21TagForm: function(MarcService2, $q) {
                                var deferred = $q.defer();
                                MarcService2.getMarc21Form().then(function(response) {
        ...
        $scope.verify = function() {
            MarcService2.verify($scope.record).then(function(response) {
                if(response.errMsg != "") {
        ...
        $scope.convert = function() {
            MarcService2.convert($scope.record).then(function(convertedRecord) {
        ...
        $scope.create = function() {
            $location.path('/cataloguing2/resourcedescriptor/create');
        }
    });
});
