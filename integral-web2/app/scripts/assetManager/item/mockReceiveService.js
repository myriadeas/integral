		...
		return {
			receive: function(receiveRequest) {
				...
				if (receiveRequest.title == "Management") {
					response = ServicesRestangular.one('assetmanager/receive/receiveResponse.json').get();
				}else{
					var deffered = $q.defer();
					ServicesRestangular.one('assetmanager/receive/error.json').get().then(function(data){
