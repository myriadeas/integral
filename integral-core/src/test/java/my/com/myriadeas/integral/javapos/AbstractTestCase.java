...
public abstract class AbstractTestCase extends JposTestCase {
	...
	private void createSimplePropFile() throws Exception {
		...
		entryList = createDefaultSerializedJposEntriesFile(POPULATOR_FILE,
				MAX_JPOSENTRIES);
	}
	...
	private void createMultiPropFile() throws Exception {
		...
		for (int i = 0; i < populatorClasses.length; ++i) {
			jclProps.put(
					JposProperties.JPOS_CONFIG_POPULATOR_CLASS_MULTIPROP_NAME
							+ "." + i, populatorClasses[i]);

			jclProps.put(
					JposProperties.JPOS_CONFIG_POPULATOR_FILE_MULTIPROP_NAME
							+ "." + i, populatorFiles[i]);

			if (populatorTypes[i].equals("XML")) {
				multiEntryLists[i] = createDefaultXmlJposEntriesFile(
						populatorFiles[i], startIndex, multiEntrySizes[i]);

				assertTrue("Could not create populator file: "
						+ populatorFiles[i],
						(new File(populatorFiles[i])).exists());
			} else {
				multiEntryLists[i] = createDefaultSerializedJposEntriesFile(
						populatorFiles[i], startIndex, multiEntrySizes[i]);
	...
	protected String[] populatorClasses = {
	...
	public static final String POPULATOR_FILE = TEST_DATA_PATH
			+ "ServiceLoaderTestCase.cfg";
}
