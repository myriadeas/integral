...
function _mandatoryFieldValidator() {
	if (mandatoryFields.length > 0) {
		return {valid: false, msg: "Mandatory tag(s): '"+ mandatoryFields + "'"};
	} else {
		return {valid:true};
	}
}
...
function verify(json) {
	...
	for(var index=0; index < fields.length; index++) {
		...
		for(var tag in field) {
			...
			if (typeof field[tag] === 'string') {
				...
				msg = mergeMessage(msg, tag + " " + varField.ind1 + varField.ind2 + " " + joinSubfields(varField.subfields));
			}
		};
	}
	...
	return {valid: valid, msg: msg, errMsg: errMsg};
}
